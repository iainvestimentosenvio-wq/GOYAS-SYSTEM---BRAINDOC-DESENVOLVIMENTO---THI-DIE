...
public sealed class PostgresAncorarPdfExecucaoRepository : IAncorarPdfExecucaoRepository
{
    ...
    public PostgresAncorarPdfExecucaoRepository(PostgresDb db, TimeProvider? timeProvider = null)
    {
        _db = db;
        ...
        DateTime mtimeUtc)
    {
        ...
        // INSERT ... ON CONFLICT DO NOTHING: atomico, idempotente.
        cmd.CommandText = @"
...
ON CONFLICT(TarefaId, CicloId, NomeEsperadoLogico) DO NOTHING;";
        ...
        return new AncorarPdfReservaIdempotencia(linhasAfetadas > 0, nomeEsperadoLogico, arquivoHash, arquivoPath);
    }
...
ON CONFLICT(ResultadoId) DO NOTHING;";
        ...
        // 3) Saida consolidada — ON CONFLICT DO NOTHING: nunca sobrescrever.
...
LIMIT @limite OFFSET @offset;";
    ...
    public IReadOnlyList<TarefaAncorarPdfExecucao> ListarPendentesParaExecucao(int clienteId, int limite = 10)
    {
        ...
        // FOR UPDATE SKIP LOCKED: padrao de fila em Postgres com multiplos workers.
        // Cada worker bloqueia so as linhas que vai processar, sem bloquear outros workers.
        using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = @"
SELECT * FROM AncorarPdfExecucoes
...
LIMIT 1;";
    ...
    private static void BindExecucao(NpgsqlCommand cmd, TarefaAncorarPdfExecucao e)
    {
        ...
        cmd.Parameters.AddWithValue("@executadaComAtraso", e.ExecutadaComAtraso);
    ...
    private static void BindResultado(NpgsqlCommand cmd, ResultadoAncoraVariavel r)
    {
    ...
    private static void BindSaida(NpgsqlCommand cmd, SaidaVariavelAncorada s)
    {
    ...
    private TarefaAncorarPdfExecucao MapExecucao(NpgsqlDataReader r)
    {
        return new TarefaAncorarPdfExecucao
        {
            ...
            ExecutadaComAtraso = r.GetBoolean(r.GetOrdinal("ExecutadaComAtraso")),
    ...
    private DateTime ReadDate(NpgsqlDataReader r, string col)
    {
        ...
        throw new FormatException($"Valor de data invalido em {nameof(PostgresAncorarPdfExecucaoRepository)}.{col}: '{raw}'.");
    }
