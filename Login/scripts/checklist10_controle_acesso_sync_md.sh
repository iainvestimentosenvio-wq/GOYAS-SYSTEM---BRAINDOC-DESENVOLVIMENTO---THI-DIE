...
START_MARKER="<!-- CHECKLIST10_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST10_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist10_controle_acesso_sync_md.sh \
    --block-file Login/testes/TestResults/checklist10_controle_acesso/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C10. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C10 sincronizado: $CHECKLIST_PATH"
