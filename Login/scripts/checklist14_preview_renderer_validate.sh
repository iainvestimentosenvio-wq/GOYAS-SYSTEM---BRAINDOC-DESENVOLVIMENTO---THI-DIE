...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist14_preview_renderer_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist14_preview_renderer_validate.sh --profile baseline --ci
  bash Login/scripts/checklist14_preview_renderer_validate.sh --profile full --ci --write-report
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist14_preview_renderer"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist14_preview_renderer"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist14_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C14_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
run_gate() {
  ...
  echo "[checklist14] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist14] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
build_core_test_command() {
  ...
  if [[ $CI_MODE -eq 1 ]]; then
    ci_env_prefix="PROTONS_CI_REQUIRE_PREVIEW_RENDERERS=1"
  fi

  printf '%s dotnet test "%s" -c Debug --filter "%s" --logger "trx;LogFileName=%s" --results-directory "%s" --artifacts-path "%s" --disable-build-servers -m:1' \
    "$ci_env_prefix" \
    "$CORE_TEST_PROJECT" \
    "$filter" \
    "$trx_file" \
...
g1_artifacts="$(gate_artifacts_path "C14_G1_Build")"
...
# --- Gate G2: Contratos Core — IPdfPreviewRenderer, modelos, NullRenderer ---
gate_cmd_g2="$(build_core_test_command "Checklist=C14&Category=C14_G2_RendererCore" "C14_G2_RendererCore.trx" "C14_G2")"

# --- Gate G3: Integracao Infra — GhostscriptRenderer, DocnetRenderer, FallbackRenderer ---
gate_cmd_g3="$(build_infra_test_command "Checklist=C14&Category=C14_G3_RendererInfra" "C14_G3_RendererInfra.trx" "C14_G3")"

# --- Gate F1 (full): Regressao Core C1-C14 ---
gate_cmd_f1="$(build_core_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|Checklist=C8|Checklist=C9|Checklist=C10|Checklist=C11|Checklist=C12|Checklist=C13|Checklist=C14" \
  "C14_F1_CoreRegression.trx" "C14_F1")"

# --- Gate F2 (full): Regressao Infra C1-C14 ---
gate_cmd_f2="$(build_infra_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|(Checklist=C8&Category=C8_G2_Guards)|Checklist=C9|Checklist=C10|Checklist=C11|Checklist=C12|Checklist=C13|Checklist=C14" \
  "C14_F2_InfraRegression.trx" "C14_F2")"

# --- Execucao dos gates blocking ---
run_gate "C14_G1_Build"      "Restore + build da solucao com warnings como erro."                                             "$gate_cmd_g1"
run_gate "C14_G2_RendererCore" "Contratos Core: IPdfPreviewRenderer, PdfRenderResult, PdfRenderMetrics, NullRenderer."         "$gate_cmd_g2"
run_gate "C14_G3_RendererInfra" "Integracao Infra: GhostscriptRenderer, DocnetRenderer (PDFium), FallbackPreviewRenderer." "$gate_cmd_g3"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C14_F1_CoreRegression"  "Regressao completa Core (C1-C14)."  "$gate_cmd_f1" "full"
  run_gate "C14_F2_InfraRegression" "Regressao completa Infra (C1-C14)." "$gate_cmd_f2" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist14_PreviewRenderer\","
  ...
  {
    echo "# Checklist 14 - Preview Renderer: IPdfPreviewRenderer (PDFium + Ghostscript Fallback)"
    echo
  ...
  echo "[checklist14] Relatorio escrito em: $summary_md"

  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$(dirname "${BASH_SOURCE[0]}")/checklist14_preview_renderer_sync_md.sh" \
      --block-file "$summary_block" \
      --checklist "$CHECKLIST_PATH"
  fi
fi

echo "[checklist14] Status geral: $overall_status"
if [[ $blocking_failures -ne 0 ]]; then
  exit 1
fi
