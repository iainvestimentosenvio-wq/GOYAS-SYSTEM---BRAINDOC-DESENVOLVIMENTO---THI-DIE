...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist12_smart_detector_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist12_smart_detector_validate.sh --profile baseline --ci
  bash Login/scripts/checklist12_smart_detector_validate.sh --profile full --ci --write-report
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist12_smart_detector"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist12_smart_detector"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist12_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C12_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
run_gate() {
  ...
  echo "[checklist12] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist12] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
build_infra_test_command() {
  ...
  if [[ $CI_MODE -eq 1 ]]; then
    ci_env_prefix="PROTONS_CI_REQUIRE_FRE_PDF=1"
  fi
...
g1_artifacts="$(gate_artifacts_path "C12_G1_Build")"
...
# --- Gate G2: Contratos do SmartDetector (sem I/O de PDF) ---
gate_cmd_g2="$(build_infra_test_command "Checklist=C12&Category=C12_G2_Contratos" "C12_G2_Contratos.trx" "C12_G2")"

# --- Gate G3: Testes com FRE PDF real ---
gate_cmd_g3="$(build_infra_test_command "Checklist=C12&Category=C12_G3_FRE" "C12_G3_FRE.trx" "C12_G3")"

# --- Gate G4: Criterio de aceite — estabilidade 5 rodadas ---
gate_cmd_g4="$(build_infra_test_command "Checklist=C12&Category=C12_G3_Estabilidade" "C12_G4_Estabilidade.trx" "C12_G4")"

# --- Gate F1 (full): Regressao Infra C1–C12 ---
gate_cmd_f1="$(build_infra_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|(Checklist=C8&Category=C8_G2_Guards)|Checklist=C9|Checklist=C10|Checklist=C11|Checklist=C12" \
  "C12_F1_InfraRegression.trx" "C12_F1")"

# --- Execucao dos gates blocking ---
run_gate "C12_G1_Build"         "Restore + build da solucao com warnings como erro."                               "$gate_cmd_g1"
run_gate "C12_G2_Contratos"     "Contratos SmartDetector sem I/O: hit-test, agrupamento, classificacao, label."    "$gate_cmd_g2"
run_gate "C12_G3_FRE"           "Deteccao no PDF FRE CAIAPONIA real: CNPJ, CPF, email, CEP, tel, data, inteiro."  "$gate_cmd_g3"
run_gate "C12_G4_Estabilidade"  "Criterio de aceite: 5 rodadas x 5 tipos = 25 deteccoes sem falha."               "$gate_cmd_g4"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C12_F1_InfraRegression" "Regressao completa Infra (C1–C12, sem load tests)." "$gate_cmd_f1" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist12_SmartDetector\","
  ...
  {
    echo "# Checklist 12 - Smart Click: Deteccao Automatica de Variaveis no PDF (validacao automatica)"
    echo
  ...
  echo "[checklist12] Relatorio escrito em: $summary_md"

  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$(dirname "${BASH_SOURCE[0]}")/checklist12_smart_detector_sync_md.sh" \
      --block-file "$summary_block" \
      --checklist "$CHECKLIST_PATH"
  fi
fi

echo "[checklist12] Status geral: $overall_status"
if [[ $blocking_failures -ne 0 ]]; then
  exit 1
fi
