...
START_MARKER="<!-- CHECKLIST07_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST07_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist07_ancorar_pdf_sync_md.sh \
    --block-file Login/testes/TestResults/checklist07_ancora_pdf/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C7. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C7 sincronizado: $CHECKLIST_PATH"
