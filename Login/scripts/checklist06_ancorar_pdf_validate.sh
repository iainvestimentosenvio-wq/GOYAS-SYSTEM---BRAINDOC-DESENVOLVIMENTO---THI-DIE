...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist06_ancorar_pdf_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist06_ancorar_pdf_validate.sh --profile baseline --ci --write-report --sync-checklist never
  bash Login/scripts/checklist06_ancorar_pdf_validate.sh --profile full --ci --write-report --sync-checklist release
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist06_ancora_pdf"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist06_ancorar_pdf"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist06_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C6_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
run_gate() {
  ...
  echo "[checklist06] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist06] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
g1_artifacts="$(gate_artifacts_path "C6_G1_Build")"
...
# --- Gate G2: Contratos Core (modelos do motor C6) ---
gate_cmd_g2="$(build_core_test_command "Checklist=C6&Category=C6_G1_Contratos" "C6_G2_Contratos.trx" "C6_G2")"

# --- Gate G3: Integracao do motor (end-to-end com SQLite + PDFs de fixture) ---
gate_cmd_g3="$(build_infra_test_command "Checklist=C6&Category=C6_G2_Motor" "C6_G3_Motor.trx" "C6_G3")"

# --- Gate G4: Estagios isolados do pipeline (extrator, seletor, validador, anchorador) ---
gate_cmd_g4="$(build_infra_test_command "Checklist=C6&Category=C6_G3_Pipeline" "C6_G4_Pipeline.trx" "C6_G4")"

# --- Full: Gate G5 — concorrencia multi-worker ---
gate_cmd_f1="$(build_infra_test_command "Checklist=C6&Category=C6_G4_Concorrencia" "C6_F1_Concorrencia.trx" "C6_F1")"

# --- Execucao dos gates blocking ---
run_gate "C6_G1_Build"           "Restore + build da solucao com warnings como erro."                                  "$gate_cmd_g1"
run_gate "C6_G2_Core_Contratos"  "Testes Core: contratos dos modelos do motor de ancoragem C6."                        "$gate_cmd_g2"
run_gate "C6_G3_Motor_Motor_Int" "Testes Integration: motor end-to-end com SQLite + PDFs de fixture."                  "$gate_cmd_g3"
run_gate "C6_G4_Pipeline_Stages" "Testes Integration: estagios isolados (extrator, seletor, validador, anchorador)."   "$gate_cmd_g4"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C6_F1_Concorrencia" "Testes Concorrencia: 2 workers, idempotencia de lease, retry Polly, timeout." "$gate_cmd_f1" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist06_AncorarPdf\","
  ...
  {
    echo "# Checklist 06 - Ancorar PDF — Motor Real (validacao automatica)"
    echo
  ...
  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$ROOT_DIR/Login/scripts/checklist06_ancorar_pdf_sync_md.sh" \
...
if [[ "$overall_status" != "PASS" ]]; then
  echo "Checklist 06 falhou. Veja logs em: $RESULTS_DIR"
  exit 1
fi

echo "Checklist 06 concluido com sucesso."
