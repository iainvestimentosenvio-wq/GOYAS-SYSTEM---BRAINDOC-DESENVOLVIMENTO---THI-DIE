...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist04_ancorar_pdf_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist04_ancorar_pdf_validate.sh --profile baseline --ci --write-report --sync-checklist never
  bash Login/scripts/checklist04_ancorar_pdf_validate.sh --profile full --ci --write-report --sync-checklist release
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist04_ancora_pdf"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist04_ancorar_pdf"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist04_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C4_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
blocking_failures=0
pending_strict=0
if [[ "${PROTONS_C4_PENDING_STRICT:-0}" == "1" || "${PROTONS_C4_PENDING_STRICT:-0}" == "true" ]]; then
  pending_strict=1
fi
...
run_gate() {
  ...
  echo "[checklist04] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist04] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
# --- G1: Build da solucao ---
g1_artifacts="$(gate_artifacts_path "C4_G1_Build")"
...
# --- G2: Testes Core (DST + Recorrencia Mensal) ---
gate_cmd_g2_dst="$(build_core_test_command      "Checklist=C4&Category=C4_G_Dst"             "C4_G2_Dst.trx"        "C4_G2_Dst")"
gate_cmd_g2_mensal="$(build_core_test_command   "Checklist=C4&Category=C4_G_RecorrenciaMensal" "C4_G2_Mensal.trx"    "C4_G2_Mensal")"
gate_cmd_g2="$(build_core_test_command          "Checklist=C4"                                 "C4_G2_Core.trx"      "C4_G2")"

# --- G3: Testes Infrastructure (Ordering + Misfire) ---
gate_cmd_g3_ordering="$(build_infra_test_command "Checklist=C4&Category=C4_G_Ordering"         "C4_G3_Ordering.trx"  "C4_G3_Ordering")"
gate_cmd_g3_misfire="$(build_infra_test_command  "Checklist=C4&Category=C4_G4_Misfire"         "C4_G3_Misfire.trx"   "C4_G3_Misfire")"
gate_cmd_g3="$(build_infra_test_command          "Checklist=C4"                                 "C4_G3_Infra.trx"     "C4_G3")"

# --- G4: Smoke operacional ---
g4_artifacts="$(gate_artifacts_path "C4_G4_Smoke_Operacional")"
gate_cmd_g4="PROTONS_DOTNET_ARTIFACTS_PATH=\"$g4_artifacts\" PROTONS_MSBUILD_OBJ_ROOT=\"$OBJ_PATH/C4_G4_smoke\" bash \"$ROOT_DIR/Login/scripts/smoke_painel.sh\""

# --- Execucao dos gates principais (blocking) ---
run_gate "C4_G1_Build"              "Restore + build da solucao com warnings como erro."                     "$gate_cmd_g1"
run_gate "C4_G2_Core_Checklist04"   "Testes Core: calculador DST e recorrencia mensal."                      "$gate_cmd_g2"
run_gate "C4_G3_Infra_Checklist04"  "Testes Infrastructure: ordenacao deterministica e misfire SQLite."      "$gate_cmd_g3"
run_gate "C4_G4_Smoke_Operacional"  "Smoke operacional do painel para C4."                                   "$gate_cmd_g4"

# --- Gates pendentes (non_blocking por padrao; strict via env PROTONS_C4_PENDING_STRICT=1) ---
pending_gate_blocking="non_blocking"
if [[ $pending_strict -eq 1 ]]; then
  pending_gate_blocking="blocking"
fi

run_gate "C4_P1" "Spring-forward (gap DST): politicas Avancar, Recuar, Pular."            "$gate_cmd_g2_dst"      "baseline" "$pending_gate_blocking"
run_gate "C4_P2" "Fall-back (overlap DST): politicas MaisCedo, MaisTarde, UmaVez."        "$gate_cmd_g2_dst"      "baseline" "$pending_gate_blocking"
run_gate "C4_P3" "Recorrencia mensal: fallback ultimo-dia-do-mes (31→30/28/29)."          "$gate_cmd_g2_mensal"   "baseline" "$pending_gate_blocking"
run_gate "C4_P4" "Recorrencia mensal: dezembro→janeiro, AgendamentoSegundo preservado."   "$gate_cmd_g2_mensal"   "baseline" "$pending_gate_blocking"
run_gate "C4_P5" "Ordenacao: PrioridadeExecucao ASC (menor numero = mais urgente)."       "$gate_cmd_g3_ordering" "baseline" "$pending_gate_blocking"
run_gate "C4_P6" "Ordenacao: FIFO por CriadoEmUtc em caso de empate de prioridade."       "$gate_cmd_g3_ordering" "baseline" "$pending_gate_blocking"
run_gate "C4_P7" "Misfire: schema v9 e tabelas AncorarPdfScheduler* persistem."           "$gate_cmd_g3_misfire"  "baseline" "$pending_gate_blocking"
run_gate "C4_P8" "Misfire: backlog criado quando atraso >= limiar configurado."            "$gate_cmd_g3_misfire"  "baseline" "$pending_gate_blocking"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C4_F1" "Misfire deduplicado: ON CONFLICT DO NOTHING para mesma janela."        "$gate_cmd_g3_misfire"  "full"
  run_gate "C4_F2" "TryMarkAsInProgress atomico: apenas um worker ganha por tarefa."       "$gate_cmd_g3_misfire"  "full"
  run_gate "C4_F3" "Resolver backlog: status atualizado, removido de pendentes."           "$gate_cmd_g3_misfire"  "full"
  run_gate "C4_F4" "Eventos do scheduler persistidos em AncorarPdfSchedulerEventos."       "$gate_cmd_g3_misfire"  "full"
  run_gate "C4_F5" "LimitePorTick: max N tarefas por ciclo do PeriodicTimer."              "$gate_cmd_g3_ordering" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist04_AncorarPdf\","
  ...
  {
    echo "# Checklist 04 - Ancorar PDF (validacao automatica)"
    echo
    ...
    echo "- C4_P strict mode: \`$([[ $pending_strict -eq 1 ]] && echo "true" || echo "false")\`"
  ...
  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$ROOT_DIR/Login/scripts/checklist04_ancorar_pdf_sync_md.sh" \
...
if [[ "$overall_status" != "PASS" ]]; then
  echo "Checklist 04 falhou. Veja logs em: $RESULTS_DIR"
  exit 1
fi

echo "Checklist 04 concluido com sucesso."
