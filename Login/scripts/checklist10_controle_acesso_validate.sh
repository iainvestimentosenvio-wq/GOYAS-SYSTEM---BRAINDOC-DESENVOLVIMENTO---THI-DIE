...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist10_controle_acesso_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist10_controle_acesso_validate.sh --profile baseline --ci --write-report --sync-checklist never
  bash Login/scripts/checklist10_controle_acesso_validate.sh --profile full --ci --write-report --sync-checklist release
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist10_controle_acesso"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist10_controle_acesso"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist10_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C10_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
run_gate() {
  ...
  echo "[checklist10] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist10] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
g1_artifacts="$(gate_artifacts_path "C10_G1_Build")"
...
# --- Gate G2: Contratos de dominio Core (C10) ---
gate_cmd_g2="$(build_core_test_command "Checklist=C10&Category=C10_G2_Contratos" "C10_G2_Core_Contratos.trx" "C10_G2")"

# --- Gate G3: Testes de repositorio Infrastructure (C10) ---
gate_cmd_g3="$(build_infra_test_command "Checklist=C10&Category=C10_G3_Repositorios" "C10_G3_Infra_Repositorios.trx" "C10_G3")"

# --- Gate G4: Testes de integracao release quality (C10) ---
gate_cmd_g4="$(build_infra_test_command "Checklist=C10&Category=C10_G4_Integracao" "C10_G4_Infra_Integracao.trx" "C10_G4")"

# --- Gate F1 (full): Regressao Core C1–C10 ---
gate_cmd_f1="$(build_core_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|Checklist=C8|Checklist=C9|Checklist=C10" \
  "C10_F1_CoreRegression.trx" "C10_F1")"

# --- Gate F2 (full): Regressao Infra C1–C10 (sem load tests C8/C9) ---
gate_cmd_f2="$(build_infra_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|(Checklist=C8&Category=C8_G2_Guards)|Checklist=C9|Checklist=C10" \
  "C10_F2_InfraRegression.trx" "C10_F2")"

# --- Execucao dos gates blocking ---
run_gate "C10_G1_Build"             "Restore + build da solucao com warnings como erro."                                "$gate_cmd_g1"
run_gate "C10_G2_Core_Contratos"    "Contratos de dominio para controle de acesso hierarquico (C10 Core)."              "$gate_cmd_g2"
run_gate "C10_G3_Infra_Repositorios" "Testes de repositorio — ResponsavelAdminId, soft-delete, redistribuicao (C10)."  "$gate_cmd_g3"
run_gate "C10_G4_Infra_Integracao"  "Testes de integracao release quality — Supremo, aprovacao, exclusao, transferencia." "$gate_cmd_g4"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C10_F1_CoreRegression"  "Regressao completa Core (C1–C10)."                          "$gate_cmd_f1" "full"
  run_gate "C10_F2_InfraRegression" "Regressao completa Infra (C1–C10, sem load tests C8/C9)."   "$gate_cmd_f2" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist10_ControleAcesso\","
  ...
  {
    echo "# Checklist 10 - Controle de Acesso (validacao automatica)"
    echo
  ...
  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$ROOT_DIR/Login/scripts/checklist10_controle_acesso_sync_md.sh" \
...
if [[ "$overall_status" != "PASS" ]]; then
  echo "Checklist 10 falhou. Veja logs em: $RESULTS_DIR"
  exit 1
fi

echo "Checklist 10 concluido com sucesso."
