...
START_MARKER="<!-- CHECKLIST13_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST13_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist13_timezone_matrix_sync_md.sh \
    --block-file Login/testes/TestResults/checklist13_timezone_matrix/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C13. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C13 sincronizado: $CHECKLIST_PATH"
