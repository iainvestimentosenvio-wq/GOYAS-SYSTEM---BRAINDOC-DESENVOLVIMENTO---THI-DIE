...
START_MARKER="<!-- CHECKLIST11_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST11_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist11_fluxo_agendamento_sync_md.sh \
    --block-file Login/testes/TestResults/checklist11_fluxo_agendamento/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C11. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C11 sincronizado: $CHECKLIST_PATH"
