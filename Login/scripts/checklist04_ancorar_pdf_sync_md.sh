...
START_MARKER="<!-- CHECKLIST04_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST04_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist04_ancorar_pdf_sync_md.sh \
    --block-file Login/testes/TestResults/checklist04_ancora_pdf/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C4. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C4 sincronizado: $CHECKLIST_PATH"
