...
START_MARKER="<!-- CHECKLIST02_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST02_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist02_ancorar_pdf_sync_md.sh \
    --block-file Login/testes/TestResults/checklist02_ancora_pdf/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C2. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C2 sincronizado: $CHECKLIST_PATH"
