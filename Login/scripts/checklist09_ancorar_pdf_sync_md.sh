...
START_MARKER="<!-- CHECKLIST09_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST09_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist09_ancorar_pdf_sync_md.sh \
    --block-file Login/testes/TestResults/checklist09_ancora_pdf/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C9. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C9 sincronizado: $CHECKLIST_PATH"
