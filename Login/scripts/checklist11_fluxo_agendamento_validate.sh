...
usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist11_fluxo_agendamento_validate.sh \
...
Exemplos:
  bash Login/scripts/checklist11_fluxo_agendamento_validate.sh --profile baseline --ci
  bash Login/scripts/checklist11_fluxo_agendamento_validate.sh --profile full --ci --write-report
USAGE
}
...
RESULTS_DIR="$ROOT_DIR/Login/testes/TestResults/checklist11_fluxo_agendamento"
RESULTS_FULL_DIR="$RESULTS_DIR/full"
TRX_DIR="$RESULTS_DIR/trx"
OBJ_ROOT="$ROOT_DIR/Login/testes/.obj_ci/checklist11_fluxo_agendamento"
ARTIFACTS_ROOT="${TMPDIR:-/tmp}/protons_checklist11_artifacts"

mkdir -p "$RESULTS_DIR" "$RESULTS_FULL_DIR" "$TRX_DIR" "$OBJ_ROOT" "$ARTIFACTS_ROOT"
rm -f "$RESULTS_DIR"/C11_*.log "$RESULTS_DIR"/summary.json "$RESULTS_DIR"/summary.md "$RESULTS_DIR"/checklist_block.md
...
run_gate() {
  ...
  echo "[checklist11] Executando ${gate_id}..."
  set +e
  {
  ...
  echo "[checklist11] ${gate_id}: ${gate_status} (${duration_seconds}s, ${gate_blocking})"
}
...
g1_artifacts="$(gate_artifacts_path "C11_G1_Build")"
...
# --- Gate G2: Contratos de fluxo (Core) ---
gate_cmd_g2="$(build_core_test_command "Checklist=C11&Category=C11_G1_FluxoContratos" "C11_G2_Core_FluxoContratos.trx" "C11_G2")"

# --- Gate G3: Seletor hibrido (Infra) ---
gate_cmd_g3="$(build_infra_test_command "Checklist=C11&Category=C11_G2_SeletorHibrido" "C11_G3_Infra_SeletorHibrido.trx" "C11_G3")"

# --- Gate G4: Wizard VM logic (Core) ---
gate_cmd_g4="$(build_core_test_command "Checklist=C11&Category=C11_G3_WizardVm" "C11_G4_Core_WizardVm.trx" "C11_G4")"

# --- Gate F1 (full): Regressao Core C1–C11 ---
gate_cmd_f1="$(build_core_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|Checklist=C8|Checklist=C9|Checklist=C10|Checklist=C11" \
  "C11_F1_CoreRegression.trx" "C11_F1")"

# --- Gate F2 (full): Regressao Infra C1–C11 ---
gate_cmd_f2="$(build_infra_test_command \
  "Checklist=C1|Checklist=C2|Checklist=C3|Checklist=C4|Checklist=C5|Checklist=C6|Checklist=C7|(Checklist=C8&Category=C8_G2_Guards)|Checklist=C9|Checklist=C10|Checklist=C11" \
  "C11_F2_InfraRegression.trx" "C11_F2")"

# --- Execucao dos gates blocking ---
run_gate "C11_G1_Build"            "Restore + build da solucao com warnings como erro."                          "$gate_cmd_g1"
run_gate "C11_G2_Core_Contratos"   "Contratos de dominio para o fluxo wizard AncorarPdf (C11 Core)."            "$gate_cmd_g2"
run_gate "C11_G3_Infra_Seletor"    "Seletor hibrido: tiebreak oldest + fallback oldest sem match (C11 Infra)."  "$gate_cmd_g3"
run_gate "C11_G4_Core_WizardVm"    "Logica pura do wizard VM: estado, guardas, staging, confirmacao SIM (C11)." "$gate_cmd_g4"

if [[ "$PROFILE" == "full" ]]; then
  run_gate "C11_F1_CoreRegression"  "Regressao completa Core (C1–C11)."                            "$gate_cmd_f1" "full"
  run_gate "C11_F2_InfraRegression" "Regressao completa Infra (C1–C11, sem load tests C8/C9)."     "$gate_cmd_f2" "full"
fi
...
if [[ $WRITE_REPORT -eq 1 || $CI_MODE -eq 1 ]]; then
  ...
  {
    echo "{"
    echo "  \"checklist\": \"Checklist11_FluxoAgendamento\","
  ...
  {
    echo "# Checklist 11 - Fluxo de Agendamento (validacao automatica)"
    echo
  ...
  echo "[checklist11] Relatorio escrito em: $summary_md"

  if [[ "$SYNC_CHECKLIST" == "release" ]]; then
    bash "$(dirname "${BASH_SOURCE[0]}")/checklist11_fluxo_agendamento_sync_md.sh" \
      --block-file "$summary_block" \
      --checklist "$CHECKLIST_PATH"
  fi
fi

echo "[checklist11] Status geral: $overall_status"
if [[ $blocking_failures -ne 0 ]]; then
  exit 1
fi
