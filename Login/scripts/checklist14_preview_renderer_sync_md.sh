...
START_MARKER="<!-- CHECKLIST14_AUTO_VALIDATION_START -->"
END_MARKER="<!-- CHECKLIST14_AUTO_VALIDATION_END -->"

usage() {
  cat <<'USAGE'
Uso:
  bash Login/scripts/checklist14_preview_renderer_sync_md.sh \
    --block-file Login/testes/TestResults/checklist14_preview_renderer/checklist_block.md \
...
if [[ $awk_exit_code -ne 0 ]]; then
  echo "Falha ao atualizar bloco automatico C14. Verifique os marcadores START/END no checklist." >&2
  exit 5
fi

mv "$tmp_file" "$CHECKLIST_PATH"
echo "Checklist C14 sincronizado: $CHECKLIST_PATH"
