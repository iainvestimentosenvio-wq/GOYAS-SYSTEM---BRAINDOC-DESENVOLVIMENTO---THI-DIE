#!/usr/bin/env bash
# run-windows-p10-gate.sh
# Runner semantico para gate P10 (resiliencia), sem suite UX.
...
usage() {
  cat <<'EOF'
Uso:
  bash run-windows-p10-gate.sh [opcoes]
...
echo "==============================================================="
echo "GATE P10 - Regressao Estendida (Sem UX)"
echo "==============================================================="
echo ""
...
if ! bash testes/windows/test-artifact-freshness.sh; then
  ...
  echo ""
  echo "ERRO: preflight de artefatos falhou. Rebuild obrigatorio antes de rodar o gate P10."
...
export PROTONS_REGRESSION_EXTRA_ARGS="-EnableResilienceSuite -RequireDefenderActive"
...
echo "Parametros de regressao P10:"
...
RUN_ID="P10-GATE-$(date -u +%Y%m%d%H%M%S)"
echo "RunId: $RUN_ID"
echo ""
