...
usage() {
  cat <<'EOF'
Uso:
  bash comum/scripts/run-max-score-safe-round.sh [opcoes]

Opcoes:
  --run-id <YYYYMMDDTHHMMSSZ>  Define RunId da rodada
  --output-dir <path>          Diretorio de saida (default: saida/validacao-max-score-<run-id>)
...
write_manifest() {
  ...
  {
    echo "# Manifesto de Evidencias - Rodada Max Score Segura"
    echo
    echo "- RunId: \`$RUN_ID\`"
...
if [ -z "$OUTPUT_DIR" ]; then
  OUTPUT_DIR="$INSTALADOR_ROOT/saida/validacao-max-score-$RUN_ID"
else
...
run_and_handle 8 "test-release-hashes" bash "$INSTALADOR_ROOT/testes/meta/test-release-hashes.sh"
run_and_handle 9 "test-code-signing-cert" bash "$INSTALADOR_ROOT/testes/meta/test-code-signing-cert.sh"
run_and_handle 10 "test-update-manifest-pinning" bash "$INSTALADOR_ROOT/testes/meta/test-update-manifest-pinning.sh"
run_and_handle 11 "test-release-approval-consistency" bash "$INSTALADOR_ROOT/testes/meta/test-release-approval-consistency.sh"
run_and_handle 12 "test-workflow-path-isolation" bash "$INSTALADOR_ROOT/testes/meta/test-workflow-path-isolation.sh"
run_and_handle 13 "test-linux-appimage" bash "$INSTALADOR_ROOT/testes/linux/test-appimage.sh"
run_and_handle 14 "test-installer-contract-static" bash "$INSTALADOR_ROOT/testes/windows/test-installer-contract-static.sh"
run_and_handle 15 "test-no-debug-symbols" bash "$INSTALADOR_ROOT/testes/meta/test-no-debug-symbols.sh"
run_and_handle 16 "test-checklist-graficos" bash "$INSTALADOR_ROOT/testes/meta/test-checklist-graficos.sh"
run_and_handle 17 "test-checklist-score-evidence" bash "$INSTALADOR_ROOT/testes/meta/test-checklist-score-evidence.sh"
run_and_handle 18 "test-go-nogo-gate" bash "$INSTALADOR_ROOT/testes/meta/test-go-nogo-gate.sh" "$RUN_ID" "$OUTPUT_DIR"
